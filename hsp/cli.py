    ...
    def __call__(self, parser, namespace, values, option_string=None):
        try:
            if len(values) == 1:
                ...
                if not file.upper().endswith("JSON") and not file.upper().endswith(
                    "YAML"
                ):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        try:
            ...
            while os.path.exists(file):
                msg = "File exists: {}.".format(file) + "Overwrite? (y/N): "
                ans = None
                ...
                if ans.upper() in ("", "N"):
                    file = (
                        input("Enter new file name " + "(default: reservation.png): ")
                        or "reservation.png"
                    )
                else:
...
def add_course_arg(subparser):
    subparser.add_argument(
        "--course", type=str, required=True, help="ID of the hochschulsport course"
    )


def add_browser_selection_group(subparser):
    browser_select = subparser.add_mutually_exclusive_group(required=False)
    ...
    browser_select.add_argument(
        "--use-chrome",
        action="store_true",
        help="Use a chrome gui session during the " + "booking process",
    )
    browser_select.add_argument(
        "--use-headless-chrome",
        action="store_true",
...
def parse_args():
    ...
    creds_parser = subparsers.add_parser(
        "check-credentials",
        help="Check " + "the validity of a provided credentials file",
    )
    add_credentials_arg(creds_parser)

    # STATUS CHECKING SUBCOMMAND
    status_parser = subparsers.add_parser(
        "course-status", help="Check the " + "status of a hochschulsport course"
    )
    ...
    booking_parser = subparsers.add_parser(
        "booking", help="book a hochschulsport course"
    )
    ...
    if not args.subcommand:
        msg = (
            "No task selected. Choose on of 'check-credentials', "
